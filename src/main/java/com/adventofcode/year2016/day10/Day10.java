...
import com.adventofcode.year2016.day9.Day9;
...
contains a value-3 microchip. In this configuration, bot number 2 is responsible for comparing value-5 microchips with value-2 microchips.
...
public class Day10 {
    public static void main(String... args) throws URISyntaxException, IOException {
        BotInspector botInspector = new BotInspector();
        ...
        Stream.of(0, 1, 2)
                .map(HolderId::valueOf)
                .map(botInspector::getOutput)
                .mapToInt(ChipValue::getValue)
                .reduce((left, right) -> left * right);

        System.out.println(Stream.of(0, 1, 2)
                .map(HolderId::valueOf)
                .map(botInspector::getOutput)
                .mapToInt(ChipValue::getValue)
                .reduce((left, right) -> left * right));
